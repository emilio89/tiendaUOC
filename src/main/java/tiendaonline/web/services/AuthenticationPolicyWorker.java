...
public class AuthenticationPolicyWorker implements ComponentClassTransformWorker2 {
	 ...
	 */
	private void processPageAnnotations(PlasticClass plasticClass, MutableComponentModel model) {

		AuthenticationPolicy policy = plasticClass.getAnnotation(AuthenticationPolicy.class);
		if (policy != null) {
			model.setMeta(AuthenticationValidator.PAGE_AUTHENTICATION_TYPE, policy.value().toString());
		}
	 ...
	 *            the transformation phase.
	 */
	private void processEventHandlerAnnotations(PlasticClass plasticClass, MutableComponentModel model) {

		for (PlasticMethod method : plasticClass.getMethodsWithAnnotation(AuthenticationPolicy.class)) {
			...
			if (methodName.startsWith("on") || event != null) {
				...
				throw new RuntimeException("Cannot put AuthenticationPolicy annotation on a non event handler method");
			}
		}
